...
#pragma mark - CHECKPOINT DATA:
    ...
    Checkpoint::Checkpoint(slice json) {
        if (json) {
            Doc root = Doc::fromJSON(json, nullptr);
            local = (C4SequenceNumber) root["local"_sl].asInt();
            remote = root["remote"_sl].toJSON();
        }
    }
    ...
    alloc_slice Checkpoint::toJSON() const {
        ...
        if (remote) {
            ...
            enc.writeRaw(remote);   // remote is already JSON
        }
        enc.endDict();
        return enc.finish();
    }
    ...
    bool Checkpoint::validateWith(const Checkpoint &remoteSequences) {
        bool match = true;
        if (local > 0 && local != remoteSequences.local) {
            LogTo(SyncLog, "Local sequence mismatch: I had %llu, remote had %llu",
                  (unsigned long long)local,
                  (unsigned long long)remoteSequences.local);
            local = 0;
            match = false;
        }
        if (remote && remote != remoteSequences.remote) {
            LogTo(SyncLog, "Remote sequence mismatch: I had '%.*s', remote had '%.*s'",
                  SPLAT(remote), SPLAT(remoteSequences.remote));
            remote = nullslice;
            match = false;
        }
        return match;
...
#pragma mark - CHECKPOINT ACCESSORS:
    ...
    Checkpoint Checkpointer::checkpoint() const {
        LOCK();
        return _chk;
    }


    void Checkpointer::setCheckpoint(const Checkpoint &chk) {
        LOCK();
        _setCheckpoint(chk);
    }


    void Checkpointer::setCheckpoint(const C4SequenceNumber *local, const slice *remote) {
        auto chk = checkpoint();
        LOCK();
        if (local)
            chk.local = *local;
    ...
    void Checkpointer::_setCheckpoint(const Checkpoint &chk) {
...
#pragma mark - PENDING DOCUMENTS:
                                          ...
                                          C4Error* outErr)
    {
        ...
        const auto replLastSequence = this->checkpoint().local;
