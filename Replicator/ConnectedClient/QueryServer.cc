...
namespace litecore::repl {
    ...
    static bool isJSONQuery(slice queryStr) { return queryStr.hasPrefix("{"); }
    ...
    C4Query* QueryServer::getNamedQuery(const string& name) {
        ...
        if ( query ) _queries.insert({name, query});
        return query;
    }
    ...
    void QueryServer::handleQuery(Retained<blip::MessageIn> request) {
        try {
            ...
            if ( !name == !src ) {
                request->respondWithError(blip::Error("HTTP", 400, "Exactly one of 'name' or 'src' must be given"));
                return;
            }
            if ( name ) {
                // Named query:
