...
namespace litecore::repl {
    ...
    class Checkpointer {
      public:
        ...
        void addPendingSequences(const std::vector<C4SequenceNumber>& sequences, C4SequenceNumber firstInRange,
                                 C4SequenceNumber lastInRange);
        void addPendingSequences(RevToSendList& sequences, C4SequenceNumber firstInRange, C4SequenceNumber lastInRange);
        ...
        slice initialCheckpointID() const {
            Assert(_initialDocID);
            return _initialDocID;
        }

        /** Returns the doc ID where the checkpoint is to be stored. */
        alloc_slice checkpointID() const {
            Assert(_docID);
            return _docID;
        }
        ...
        static bool getPeerCheckpoint(C4Database* NONNULL, slice checkpointID, alloc_slice& outBody,
                                      alloc_slice& outRevID);

        /// Returns false if revID conflicts.
        static bool savePeerCheckpoint(C4Database* NONNULL, slice checkpointID, slice body, slice revID,
      ...
      private:
        ...
        static alloc_slice _read(C4Database* db NONNULL, slice);
