...
#pragma mark - CONFLICTS & OUT-OF-ORDER CHANGES:
    ...
    bool Pusher::shouldRetryConflictWithNewerAncestor(RevToSend *rev) {
        ...
        bool retry = false;
        C4Error error;
        c4::ref<C4Document> doc = c4db_getDoc(_db->useLocked(), rev->docID, true, kDocGetAll, &error);
    ...
    void Pusher::_docRemoteAncestorChanged(alloc_slice docID, alloc_slice foreignAncestor) {
        ...
        if (i != _conflictsIMightRetry.end()) {
            ...
            c4::ref<C4Document> doc = c4doc_getBySequence(_db->useLocked(), rev->sequence, nullptr);
            if (!doc || !c4rev_equal(doc->revID, rev->revID)) {
