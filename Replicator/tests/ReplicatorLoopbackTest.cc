...
TEST_CASE_METHOD(ReplicatorLoopbackTest, "Push Obsoleted Revs", "[Push]") {
    importJSONLines(sFixturesDir + "names_100.json");
    _expectedDocumentCount = 100;
    addRevsInParallel(chrono::milliseconds(15), alloc_slice("0000080"), 2, 1);
    runPushReplication();
    compareDatabases();
    validateCheckpoints(db, db2, "{\"local\":101}");
}
