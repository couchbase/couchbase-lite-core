...
static void validateCipherInputs(ReplicatorSGTest::CipherContext* ctx, C4String& docID, C4String& keyPath) {
    CHECK(docID == ctx->docID);
    CHECK(keyPath == ctx->keyPath);
    ctx->called = true;
}

C4SliceResult propEncryptor(void* ctx, C4String docID, FLDict properties,
                            C4String keyPath, C4Slice input, C4StringResult* outAlgorithm,
                            C4StringResult* outKeyID, C4Error* outError)
{
    auto test = static_cast<ReplicatorSGTest*>(ctx);
    validateCipherInputs(test->encContext.get(), docID, keyPath);
    return C4SliceResult(ReplicatorLoopbackTest::UnbreakableEncryption(input, 1));
}

C4SliceResult propDecryptor(void* ctx, C4String docID, FLDict properties,
                            C4String keyPath, C4Slice input, C4String algorithm,
                            C4String keyID, C4Error* outError)
{
    auto test = static_cast<ReplicatorSGTest*>(ctx);
    validateCipherInputs(test->decContext.get(), docID, keyPath);
    return C4SliceResult(ReplicatorLoopbackTest::UnbreakableEncryption(input, -1));
}
...
// Jianmin will handle the refactoring of these in his CBL-4101 PR
#if 0
