...
TEST_CASE_METHOD(ReplicatorWalrusTest, "Pull deltas with filter from SG", "[.SyncServerWalrus][Delta]") {
...
TEST_CASE_METHOD(ReplicatorWalrusTest, "Push deltas to SG", "[.SyncServerWalrus][Delta]") {
    static constexpr int kNumDocs = 10, kNumProps = 30;
    flushScratchDatabase();
    _logRemoteRequests = false;
    ...
    for (int docNo = 0; docNo < kNumDocs; ++docNo) {
        ...
        TransactionHelper t(db);
        C4Error error;
        c4::ref<C4Document> doc = c4doc_get(db, docIDsl, false, ERROR_INFO(error));
        REQUIRE(doc);
        Dict props = c4doc_getProperties(doc);
        Encoder enc(c4db_createFleeceEncoder(db));
