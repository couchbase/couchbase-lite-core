...
ReplParams::ReplParams(const std::vector<C4CollectionSpec>& collSpecs, C4ReplicatorMode push, C4ReplicatorMode pull)
    ...
    for ( int i = 0; i < collSpecs.size(); ++i ) { _collectionVector[i] = {collSpecs[i], push, pull}; }
    collections       = _collectionVector.data();
    collectionCount   = _collectionVector.size();
    _optionsDict      = {};
    optionsDictFleece = _optionsDict.data();
...
void ReplParams::addCollections(const std::vector<C4ReplicationCollection>& collectionsToAdd) {
    for ( const auto& c : collectionsToAdd ) { _collectionVector.push_back(c); }
    collections     = _collectionVector.data();
    collectionCount = _collectionVector.size();
}
...
ReplParams& ReplParams::setCollectionOptions(C4CollectionSpec collectionSpec, const AllocedDict& options) {
    for ( auto& c : _collectionVector ) {
        if ( c.collection == collectionSpec ) {
            _collectionsOptionsDict.emplace_back(setOptions(AllocedDict(c.optionsDictFleece), options));
...
ReplParams& ReplParams::setDocIDs(const std::vector<std::unordered_map<alloc_slice, uint64_t>>& docIDs) {
    for ( size_t i = 0; i < docIDs.size(); ++i ) {
        ...
        for ( const auto& d : docIDs[i] ) { enc.writeString(d.first); }
        enc.endArray();
        Doc doc{enc.finish()};
        _collectionsOptionsDict.emplace_back(repl::Options::updateProperties(
                AllocedDict(_collectionVector[i].optionsDictFleece), kC4ReplicatorOptionDocIDs, doc.root()));
...
C4ReplicatorParameters ReplParams::applyThisTo(C4ReplicatorParameters params) {
    ...
    if ( onStatusChanged ) params.onStatusChanged = onStatusChanged;
    if ( onDocumentsEnded ) params.onDocumentsEnded = onDocumentsEnded;
    if ( onBlobProgress ) params.onBlobProgress = onBlobProgress;
    if ( propertyEncryptor ) params.propertyEncryptor = propertyEncryptor;
    if ( propertyDecryptor ) params.propertyDecryptor = propertyDecryptor;
    if ( callbackContext ) params.callbackContext = callbackContext;
    if ( socketFactory ) params.socketFactory = socketFactory;

    return params;
}

std::function<void(C4ReplicatorParameters&)> ReplParams::paramSetter() {
    return [&](C4ReplicatorParameters& c4Params) { c4Params = applyThisTo(c4Params); };
}

ReplParams& ReplParams::setPushFilter(ValidationFunction pushFilter) {
    for ( auto& c : _collectionVector ) { c.pushFilter = pushFilter; }
    return *this;
}

ReplParams& ReplParams::setPullFilter(ValidationFunction pullFilter) {
    for ( auto& c : _collectionVector ) { c.pullFilter = pullFilter; }
    return *this;
}
...
ReplParams& ReplParams::setCallbackContext(void* callbackContext_) {
    for ( auto& c : _collectionVector ) { c.callbackContext = callbackContext_; }
    return *this;
}
