...
bool SG::createUser(const std::string& username, const std::string& password) const {
    ...
    deleteUser(username);
    (void)runRequest("POST", {}, "_user", body, true, nullptr, &status);
    return status == HTTPStatus::Created;
}

bool SG::deleteUser(const std::string& username) const {
    HTTPStatus status;
    (void)runRequest("DELETE", {}, "_user/"s + username, nullslice, true, nullptr, &status);
...
bool SG::assignUserChannel(const std::string& username, const std::vector<C4CollectionSpec>& collectionSpecs,
    ...
    HTTPStatus status;
    (void)runRequest("PUT", {}, "_user/"s + username, v.toJSON(), true, nullptr, &status);
    return status == HTTPStatus::OK;
}
...
bool SG::upsertDoc(C4CollectionSpec collectionSpec, const std::string& docID, slice body,
    ...
    HTTPStatus status;
    (void)runRequest("PUT", collectionSpec, docID, channelIDs.empty() ? body : bodyWithChannel, false, err, &status);
...
bool SG::upsertDocWithEmptyChannels(C4CollectionSpec collectionSpec, const std::string& docID, slice body,
    ...
    HTTPStatus status;
    (void)runRequest("PUT", collectionSpec, docID, bodyWithChannel, false, err, &status);
    return status == HTTPStatus::OK || status == HTTPStatus::Created;
}
...
void SG::flushDatabase() const { (void)runRequest("POST", {}, "_flush", nullslice, true); }

bool SG::insertBulkDocs(C4CollectionSpec collectionSpec, const slice docsDict, double timeout) const {
    HTTPStatus status;
    (void)runRequest("POST", collectionSpec, "_bulk_docs", docsDict, false, nullptr, &status, timeout, false);
    return status == HTTPStatus::Created;
}
