...
public:
    ...
    std::vector<C4Collection*>
    collectionPreamble(const std::vector<C4CollectionSpec>& collectionSpecs) {
        std::vector<C4Collection*> ret { collectionSpecs.size() };
        for (size_t i = 0; i < collectionSpecs.size(); ++i) {
            if (kC4DefaultCollectionSpec != collectionSpecs[i]) {
                db->createCollection(collectionSpecs[i]);
            }
    ...
    void verifyDocs(const std::vector<std::unordered_map<alloc_slice, unsigned>>& docIDs,
        ...
        for (size_t i = 0; i < _collectionCount; ++i) {
            if (_collectionSpecs[i] != Default) {
                verifyDb->createCollection(_collectionSpecs[i]);
            }
        ...
        for (size_t i = 0; i < _collectionCount; ++i) {
            replCollections[i] =
                    C4ReplicationCollection{_collectionSpecs[i], kC4Disabled, kC4OneShot};
        }
