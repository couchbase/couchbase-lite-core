...
TEST_CASE_METHOD(ReplicatorAPITest, "API Create C4Replicator without start", "[C][Push]") {
    ...
    _sgConnection.remoteDBName = "something"_sl;

    _repl = c4repl_new(db, _sgConnection.address, _sgConnection.remoteDBName, params, ERROR_INFO(err));
    CHECK(_repl);
...
TEST_CASE_METHOD(ReplicatorAPITest, "API Invalid Scheme", "[C][Push][!throws]") {
    ExpectingExceptions x;
    _sgConnection.address.scheme = "http"_sl;
    C4Error err;
    CHECK(!c4repl_isValidRemote(_sgConnection.address, _sgConnection.remoteDBName, nullptr));
...
TEST_CASE_METHOD(ReplicatorAPITest, "API Invalid URLs", "[C][Push][!throws]") {
    ...
    _sgConnection.remoteDBName = "Invalid Name"_sl;
    err = {};
    CHECK(!c4repl_isValidRemote(_sgConnection.address, _sgConnection.remoteDBName, nullptr));
    REQUIRE(!startReplicator(kC4Disabled, kC4OneShot, &err));
    CHECK(err.domain == NetworkDomain);
