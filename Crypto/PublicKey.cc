...
namespace litecore::crypto {
            ...
            case KeyFormat::PEM:
                {
                    ...
                    if ( format == KeyFormat::PEM ) result = convertToPEM(result, "PUBLIC KEY");
                    return result;
                }
            case KeyFormat::Raw:
    ...
    PublicKey::PublicKey(slice data) { parsePEMorDER(data, "public key", context(), &mbedtls_pk_parse_public_key); }
    ...
    ExternalPrivateKey::ExternalPrivateKey(unsigned keySizeInBits) : _keyLength((keySizeInBits + 7) / 8) {
