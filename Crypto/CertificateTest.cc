...
TEST_CASE("Creating subject names", "[Certs]") {
    DistinguishedName name(
            {{"CN"_sl, "Jane Doe"_sl}, {"O"_sl, "ExampleCorp"_sl}, {"C"_sl, "US"_sl}, {"pseudonym"_sl, "3Jane"_sl}});
    ...
    name = DistinguishedName({{"CN"_sl, "Jane Doe, MD"_sl}});
    CHECK(name == "CN=Jane Doe\\, MD"_sl);
    CHECK(name["CN"_sl] == "Jane Doe, MD"_sl);
    CHECK(name["foo"_sl] == nullslice);

    name = DistinguishedName({{"CN"_sl, ",Jane,,Doe,"_sl}});
...
TEST_CASE("Self-signed cert with Subject Alternative Name", "[Certs]") {
    ...
    subjectParams.subjectAltNames.emplace_back(SANTag::kRFC822Name, "jane@example.com"_sl);
    subjectParams.subjectAltNames.emplace_back(SANTag::kDNSName, "https://example.com/jane/"_sl);
    ...
    SECTION("Self-signed") { cert = new Cert(subjectParams, issuerParams, key); }
...
TEST_CASE("Persistent save duplicate cert or id", "[Certs]") {
    ...
    ExpectException(error::LiteCore, error::CryptoError, [&] { cert1->save("cert1", true); });

#    ifdef __APPLE__
    // Save cert1 again with a different id:
    ExpectException(error::LiteCore, error::CryptoError, [&] { cert1->save("cert2", true); });
#    endif
