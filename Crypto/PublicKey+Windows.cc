...
namespace litecore::crypto {
    ...
    inline void checkSecurityStatus(SECURITY_STATUS err, const char* fnName, const char* what,
        ...
        if ( err != ERROR_SUCCESS ) {
            if ( objectToFree != 0 ) { NCryptFreeObject(objectToFree); }
    ...
    PCCERT_CHAIN_CONTEXT getCertChain(PCCERT_CONTEXT leaf) {
        ...
        BOOL success = CertGetCertificateChain(HCCE_CURRENT_USER, leaf, NULL, NULL, &para, 0, NULL, &retVal);
    ...
    static int getChildCount(HCERTSTORE store, PCCERT_CONTEXT cert) {
        ...
        do {
            ++certCount;
            childCert = CertFindCertificateInStore(store, X509_ASN_ENCODING, 0, CERT_FIND_ISSUER_STR_W, &subjectName,
                                                   childCert);
        } while ( childCert );
    ...
    static NCRYPT_PROV_HANDLE openStorageProvider() {
        ...
        SECURITY_STATUS    result = NCryptOpenStorageProvider(&hProvider, MS_PLATFORM_CRYPTO_PROVIDER, 0);
        if ( result == ERROR_SUCCESS ) { return hProvider; }
    ...
    class NCryptPrivateKey : public PersistentPrivateKey {
      public:
        ...
        NCryptPrivateKey(unsigned keySizeInBits, NCRYPT_KEY_HANDLE keyPair)
            : PersistentPrivateKey(keySizeInBits), _keyPair(keyPair) {
            Assert(_keyPair);
        }

        ~NCryptPrivateKey() { NCryptFreeObject(_keyPair); }

        static alloc_slice publicKeyRawData(const NCRYPT_KEY_HANDLE hKey) {
            DWORD bytesNeeded = 0;

            SECURITY_STATUS result = NCryptExportKey(hKey, NULL, BCRYPT_RSAPUBLIC_BLOB, NULL, nullptr, 0, &bytesNeeded,
                                                     NCRYPT_SILENT_FLAG);
            ...
            result = NCryptExportKey(hKey, NULL, BCRYPT_RSAPUBLIC_BLOB, nullptr, pkBytes, bytesNeeded, &bytesNeeded,
                                     NCRYPT_SILENT_FLAG);
            ...
            encodeResult = CryptEncodeObject(X509_ASN_ENCODING, CNG_RSA_PUBLIC_KEY_BLOB, pkInfo, PBYTE(retVal.buf),
                                             &bytesNeeded);
        ...
        alloc_slice publicKeyRawData() override { return publicKeyRawData(_keyPair); }
      ...
      protected:
        int _decrypt(const void* input, void* output, size_t output_max_len, size_t* output_len) noexcept override {
            const auto result = NCryptDecrypt(_keyPair, PBYTE(input), _keyLength, nullptr, PBYTE(output),
                                              narrow_cast<DWORD>(output_max_len), reinterpret_cast<DWORD*>(output_len),
            ...
            if ( result != ERROR_SUCCESS ) {
                ...
                return result == NTE_BUFFER_TOO_SMALL ? MBEDTLS_ERR_RSA_OUTPUT_TOO_LARGE
        ...
        int _sign(int /*mbedtls_md_type_t*/ mbedDigestAlgorithm, slice inputData,
            ...
            if ( kDigestAlgorithmMap.find(mbedDigestAlgorithm) != kDigestAlgorithmMap.end() )
    ...
    Retained<PersistentPrivateKey> PersistentPrivateKey::generateRSA(unsigned keySizeInBits) {
        ...
        SECURITY_STATUS    result = NCryptCreatePersistedKey(hProvider, &hKey, NCRYPT_RSA_ALGORITHM, wtimestr, 0, 0);
    ...
    Retained<PersistentPrivateKey> PersistentPrivateKey::withPublicKey(PublicKey* publicKey) {
        ...
        do {
            ...
            result = NCryptEnumKeys(hProvider, NULL, &next, &enumState, 0);

            if ( result != ERROR_SUCCESS ) { break; }

            SECURITY_STATUS openResult = NCryptOpenKey(hProvider, &hKey, next->pszName, 0, 0);
    ...
    Retained<PersistentPrivateKey> Cert::loadPrivateKey() { return PersistentPrivateKey::withCertificate(this); }
