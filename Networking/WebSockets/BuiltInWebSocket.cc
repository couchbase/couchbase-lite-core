...
namespace litecore::websocket {
    ...
    void BuiltInWebSocket::requestClose(int status, fleece::slice message) { Assert(false, "Should not be called"); }
...
#pragma mark - CONNECTING:
    ...
    unique_ptr<ClientSocket> BuiltInWebSocket::_connectLoop() {
        ...
        if ( !configureProxy(logic, options()[kC4ReplicatorOptionProxyServer].asDict()) ) {
            closeWithError(
                    c4error_make(LiteCoreDomain, kC4ErrorInvalidParameter, "Invalid/unsupported proxy settings"_sl));
            return nullptr;
        }
        ...
        do {
            ...
            switch ( lastDisposition ) {
                case HTTPLogic::kSuccess:
                    ...
                    break;
                case HTTPLogic::kRetry:     // redirected; go around again
                case HTTPLogic::kContinue:  // Will continue with the same socket (after connecting to a proxy)
                    break;
    ...
    alloc_slice BuiltInWebSocket::cookiesForRequest(const Address &addr) {
        alloc_slice cookies;
        if (_database)
            cookies = _database->useLocked()->getCookies(addr);
    ...
    void BuiltInWebSocket::writeToSocket() {
        try {
            ...
            bool   moreToWrite;
            {
                ...
                unique_lock<mutex> lock(_outboxMutex);
                _outboxAlloced.erase(_outboxAlloced.begin(), _outboxAlloced.begin() + narrow_cast<long>(nRemoved));
                _outbox.erase(_outbox.begin(), _outbox.begin() + narrow_cast<long>(nRemoved));
