...
namespace litecore { namespace websocket {
        ...
        class MessageImpl : public Message {
          public:
            ...
            ~MessageImpl() { _webSocket->receiveComplete(_size); }
        ...
        WebSocketImpl::WebSocketImpl(const URL& url, Role role, bool framing, const Parameters& parameters)
            : WebSocket(url, role)
            , Logging(WSLogDomain)
            ...
            if ( framing ) {
                if ( role == Role::Server ) _serverProtocol.reset(new ServerProtocol);
                else
        ...
        bool WebSocketImpl::sendOp(fleece::slice message, int opcode) {
            ...
            bool        writeable;
            {
                ...
                if ( _framing ) {
                    ...
                    size_t newSize;
                    if ( role() == Role::Server ) {
                        newSize = ServerProtocol::formatMessage((char*)frame.buf, (const char*)message.buf,
                                                                message.size, (uWS::OpCode)opcode, message.size, false);
                    } else {
                        newSize = ClientProtocol::formatMessage((char*)frame.buf, (const char*)message.buf,
                                                                message.size, (uWS::OpCode)opcode, message.size, false);
                    }
        ...
        void WebSocketImpl::onReceive(slice data) {
            ...
            alloc_slice msgToSend;
            {
                ...
                if ( _framing ) {
                    ...
                    if ( _clientProtocol ) _clientProtocol->consume((const char*)data.buf, (unsigned)data.size, this);
                    else
        ...
        bool WebSocketImpl::handleFragment(char* data, size_t length, unsigned int remainingBytes, int opCode,
                                           bool fin) {
            // Beginning:
            if ( !_curMessage ) {
                _curOpCode = opCode;
                ...
                case SOCKET_OPENED:
                    ...
                    if ( _framing ) {
                        alloc_slice closeMsg;
                        {
                            ...
                            auto size = ClientProtocol::formatClosePayload((char*)closeMsg.buf, (uint16_t)status,
                ...
                if ( _didConnect ) {
                    ...
                    if ( _framing ) {
                        ...
                        if ( expected && clean ) logInfo("Socket disconnected cleanly");
                            ...
                            logError("Unexpected or unclean socket disconnect! (reason=%-s %d)", status.reasonName(),
                                     status.code);
                        ...
                        if ( clean ) logInfo("WebSocket closed normally");
                            ...
                            logError("WebSocket closed abnormally (reason=%-s %d)", status.reasonName(), status.code);
                    }

                    _timeConnected.stop();
                    double t = _timeConnected.elapsed();
                    logInfo("sent %" PRIu64 " bytes, rcvd %" PRIu64 ", in %.3f sec (%.0f/sec, %.0f/sec)", _bytesSent,
                            _bytesReceived, t, _bytesSent / t, _bytesReceived / t);
                } else {
                    logError("WebSocket failed to connect! (reason=%-s %d)", status.reasonName(), status.code);
                }
            }
...
#define _sock ((litecore::websocket::WebSocketImpl*)user)
    ...
    template <const bool isServer>
    bool WebSocketProtocol<isServer>::handleFragment(char* data, size_t length, unsigned int remainingByteCount,
                                                     int opcode, bool fin, void* user) {
