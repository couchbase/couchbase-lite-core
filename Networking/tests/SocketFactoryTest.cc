//
// Created by Jens Alfke on 12/19/25.
//
...
class DummyC4Socket final
    : public RefCounted
    , public C4Socket
    , public Waitable {
  public:
    DummyC4Socket(C4SocketFactory const& factory, void* nativeHandle, bool isServer_)
        : C4Socket{factory, nativeHandle}, isServer(isServer_) {
        if ( nativeHandle && _factory.attached ) _factory.attached(this);
    }
    ...
    void send(slice data) { _factory.write(this, C4SliceResult(alloc_slice(data))); }

    void close() { _factory.close(this); }
    ...
    void waitFor(bool const& flag, chrono::seconds timeout = 10s) override { Waitable::waitFor(flag, timeout); }
    ...
    void received(slice data) override {
        unique_lock lock(_mutex);
        if ( data.size > 0 ) dataRead += string(data);
        else
            didReadEOF = true;
        _cond.notify_all();
    }
...
struct SocketFactoryTestDelegate
    : public REST::Server::Delegate
    , public Waitable {
