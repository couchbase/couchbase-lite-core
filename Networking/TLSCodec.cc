...
namespace litecore::net {
    ...
    public:
        ...
        TLSSocket(const C4SocketFactory& platformFactory,
                  void* platformNativeHandle,
        ...
        std::string_view hostname() const {
            if (!_url.empty()) {
                C4Address addr;
                if (C4Address::fromURL(_url, &addr, nullptr))
                    return slice(addr.hostname);
            }
            return "";
        }
        ...
        void closed(C4Error errorIfAny) override {
            ...
            if (_state != State::closed) {
                if (_error)
                    logError("Closed, with error %s", _error.description().c_str());
                else
                    logInfo("Closed");
                _state = State::closed;
                _bio.closed();
    ...
    protected:
        ...
        void open(C4Socket* socket, const C4Address& addr, C4Slice options) override {
            ...
            if (slice(plainAddr.scheme).hasSuffix('s'))
                --plainAddr.scheme.size;
            if (plainAddr.port == 0)
                plainAddr.port = 443;
    ...
    private:
        ...
        void processData() {
            ...
            do {
                ...
                }
                {
                    ...
                    int n;
                    if (!_cleartextRecvBuffer)
                        _cleartextRecvBuffer = make_unique<uint8_t[]>(kBufferSize);
        ...
        bool verifyPeer() {
            ...
            if (verify_flags != 0 && verify_flags != 0xFFFFFFFF && !(verify_flags & MBEDTLS_X509_BADCERT_SKIP_VERIFY)) {
                char vrfy_buf[512];
        ...
        private:
            int recv(void* buf, size_t length) {
                LogDebug(websocket::WSLogDomain, "mbedTLS wants to read %zu bytes; %zu available", length, _recvBuffer.size());
                if (_closed)
                    return MBEDTLS_ERR_NET_CONN_RESET;
                if (_recvBuffer.empty())
                    return _readEOF ? 0 : MBEDTLS_ERR_SSL_WANT_READ;
        ...
        [[nodiscard]] BIO::Result getBioResult() {
            ...
            if (_state == State::closed) {
                ...
                if (_state == State::closing && _pendingDownstreamWrites == 0)
                    _factory.close(this);
            }
            return result;
        }
        ...
        void performBIO(BIO::Result& bioResult) {
            ...
            if (bioResult.bytesRead > 0)
                _factory.completedReceive(this, bioResult.bytesRead);
            if (bioResult.toWrite.size > 0)
                _factory.write(this, C4SliceResult(std::move(bioResult.toWrite)));
        }
    ...
    pair<C4SocketFactory,void*> wrapSocketInTLS(const C4SocketFactory& factory,
                                                void* nativeHandle,
                                                TLSContext* tlsContext)
    {
