...
public:
    ...
    ~CertHelper() {
        if ( _serverPersistentIdentity.key ) { c4keypair_removePersistent(_serverPersistentIdentity.key.get(), nullptr); }
        if ( _clientPersistentIdentity.key ) { c4keypair_removePersistent(_clientPersistentIdentity.key.get(), nullptr); }
    }
#endif
