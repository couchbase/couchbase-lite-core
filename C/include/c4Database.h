...
C4API_BEGIN_DECLS
    ...
    /** Stores a password into a C4EncryptionKey, by using the key-derivation algorithm PBKDF2, with 
        SHA1 being the hashing function, to securely convert the password into a raw binary key.
        @param encryptionKey  The raw key will be stored here.
        @param keyLength the length of encrptionKey. As input, it gives the length of output buffer,
           encryptionKey. As output, it will be the actual length required, or filled up, by the algorithm.
           If the required length is greater the given length, this function returns false.
           32 is a good start.
        @param password  The password string.
        @return  True on success, false on failure */
    bool c4key_setPasswordSHA1(uint8_t* C4NONNULL encryptionKey,
                              size_t*  C4NONNULL keyLength,
                              C4String password) C4API;
