...
C4API_BEGIN_DECLS
    ...
    CBL_CORE_API bool c4key_setPassword(C4EncryptionKey *encryptionKey,
                           C4String password,
    ...
    /** Stores a password into a C4EncryptionKey, by using the key-derivation algorithm PBKDF2
        ...
        @param alg  The encryption algorithm to use. Must not be kC4EncryptionNone.
        @return  True on success, false on failure */
    CBL_CORE_API bool c4key_setPasswordSHA1(C4EncryptionKey *encryptionKey,
                               C4String password,
                               C4EncryptionAlgorithm alg) C4API;
    ...
    CBL_CORE_API C4Database* c4db_openNamed(C4String name,
    ...
    CBL_CORE_API C4Database* c4db_openAgain(C4Database* db,
                               C4Error* C4NULLABLE outError) C4API;
    ...
    /** Copies a prebuilt database from the given source path and places it in the destination
        ...
        @return  True on success, false on failure. */
    CBL_CORE_API bool c4db_copyNamed(C4String sourcePath,
    ...
    CBL_CORE_API bool c4db_deleteNamed(C4String dbName,
                          C4String inDirectory,
                          C4Error* C4NULLABLE outError) C4API;


    /** Changes a database's encryption key (removing encryption if it's NULL.) */
    CBL_CORE_API bool c4db_rekey(C4Database* database,
    ...
    CBL_CORE_API bool c4db_getUUIDs(C4Database* database,
    ...
    CBL_CORE_API bool c4db_maintenance(C4Database* database,
   ...
   /** @} */
    ...
    CBL_CORE_API bool c4db_beginTransaction(C4Database* database,
                               C4Error* C4NULLABLE outError) C4API;
    ...
    CBL_CORE_API bool c4db_endTransaction(C4Database* database,
                             bool commit,
                             C4Error* C4NULLABLE outError) C4API;
    ...
    CBL_CORE_API C4RawDocument* c4raw_get(C4Database* database,
                             C4String storeName,
                             C4String docID,
                             C4Error* C4NULLABLE outError) C4API;

    /** Writes a raw document to the database, or deletes it if both meta and body are NULL. */
    CBL_CORE_API bool c4raw_put(C4Database* database,
                   C4String storeName,
                   C4String key,
                   C4String meta,
                   C4String body,
