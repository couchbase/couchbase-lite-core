...
C4Cert::C4Cert(CertBase* impl) : _impl(impl) { Assert(impl); }

C4Cert::~C4Cert() = default;

#endif  // COUCHBASE_ENTERPRISE
Cert* C4Cert::asSignedCert() { return _impl->isSigned() ? (Cert*)_impl.get() : nullptr; }
...
alloc_slice C4Cert::getChainData() {
    if ( auto signedCert = asSignedCert(); signedCert ) return C4SliceResult(signedCert->dataOfChain());
    else
...
alloc_slice C4Cert::getSubjectNameComponent(C4CertNameAttributeID attrID) {
    if ( auto tag = SubjectAltNames::tagNamed(attrID); tag ) return _impl->subjectAltNames()[*tag];
    else
...
C4Cert::NameInfo C4Cert::getSubjectNameAtIndex(unsigned index) {
    ...
    if ( auto san = _impl->subjectAltNames(); index < san.size() ) {
        return {alloc_slice(SubjectAltNames::nameOfTag(san[index].first)), alloc_slice(san[index].second)};
    }
...
#    ifdef PERSISTENT_PRIVATE_KEY_AVAILABLE
    if ( auto key = assertSignedCert()->loadPrivateKey(); key ) return new C4KeyPair(key);
    return nullptr;
#    else
...
Retained<C4Cert> C4Cert::getNextInChain() {
    if ( auto signedCert = asSignedCert(); signedCert ) {
        if ( auto next = signedCert->next(); next ) return new C4Cert(next);
    }
    return nullptr;
}
...
Retained<C4Cert> C4Cert::createRequest(const std::vector<C4CertNameComponent>& nameComponents, C4CertUsage certUsages,
    ...
    for ( auto& component : nameComponents ) {
        ...
        if ( auto tag = SubjectAltNames::tagNamed(attributeID); tag ) altNames.emplace_back(*tag, component.value);
        else
...
#    ifdef ENABLE_SENDING_CERT_REQUESTS
    auto internalCallback = [=](crypto::Cert* cert, C4Error error) {
        ...
        if ( cert ) c4cert = new C4Cert(cert);
        callback(c4cert, error);
    };
    auto request = retained(new litecore::REST::CertRequest());
    request->start(assertUnsignedCert(), net::Address(address), AllocedDict(optionsDictFleece), internalCallback);
#    else
...
#    ifdef PERSISTENT_PRIVATE_KEY_AVAILABLE
    if ( auto cert = Cert::loadCert(string(name)) ) return new C4Cert(cert);
    else
        return nullptr;
#    else
    C4Error::raise(LiteCoreDomain, kC4ErrorUnimplemented, "No persistent key support");
...
C4KeyPair::C4KeyPair(Key* key) : _impl(key) { Assert(key); }
...
PrivateKey* C4NULLABLE C4KeyPair::getPrivateKey() { return _impl->isPrivate() ? (PrivateKey*)_impl.get() : nullptr; }

PersistentPrivateKey* C4KeyPair::getPersistentPrivateKey() {
    if ( PrivateKey* priv = getPrivateKey(); priv ) return priv->asPersistent();
    return nullptr;
}
...
bool C4KeyPair::hasPrivateKey() { return getPrivateKey() != nullptr; }

alloc_slice C4KeyPair::getPublicKeyDigest() { return alloc_slice(_impl->digestString()); }

alloc_slice C4KeyPair::getPublicKeyData() { return _impl->publicKeyData(); }

alloc_slice C4KeyPair::getPrivateKeyData() {
    if ( auto priv = getPrivateKey(); priv && priv->isPrivateKeyDataAvailable() ) return priv->privateKeyData();
    return nullslice;
}
...
#    ifdef PERSISTENT_PRIVATE_KEY_AVAILABLE
    if ( auto persistent = c4Key->getPersistentPrivateKey(); persistent ) return new C4KeyPair(persistent);
...
#    ifdef PERSISTENT_PRIVATE_KEY_AVAILABLE
    if ( auto persistentKey = priv->asPersistent(); persistentKey ) persistentKey->remove();
#    endif
}
...
namespace litecore {
    ...
    class ExternalKeyPair : public ExternalPrivateKey {
      public:
        ExternalKeyPair(size_t keySizeInBits, void* externalKey, struct C4ExternalKeyCallbacks callbacks)
