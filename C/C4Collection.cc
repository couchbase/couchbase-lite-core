...
#pragma mark - TRANSACTIONS:
        ...
        bool changedDuringTransaction() override {
            return _sequenceTracker && _sequenceTracker->useLocked()->changedDuringTransaction();
        }


        void transactionEnding(ExclusiveTransaction *transaction, bool committing) override {
            if (_sequenceTracker) {
...
#pragma mark - DOCUMENTS:
        ...
        virtual Retained<C4Document> newDocumentInstance(const litecore::Record &record) override {
...
#pragma mark - PURGING / EXPIRING DOCS:
        ...
        bool stopHousekeeping() override {
            if (!_housekeeper)
                return false;
            _housekeeper->stop();
            _housekeeper = nullptr;
...
Retained<C4Collection> C4Collection::newCollection(C4Database *db, slice name, KeyStore &store) {
    return new CollectionImpl(db, name, store);
}
