...
C4StringResult c4_getBuildInfo() C4API {
    ...
    if (strcmp(GitBranch, "HEAD") == 0)
        return sliceResult(format("built from commit %.8s%s on %s %s",
                                  GitCommit, GitDirty, __DATE__, __TIME__));
    else
        return sliceResult(format("built from %s branch, commit %.8s%s on %s %s",
                                  GitBranch, GitCommit, GitDirty, __DATE__, __TIME__));
#else
    return sliceResult("build info unknown");
#endif
}
