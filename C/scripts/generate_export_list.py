...
def write_def_file(cbl_export_list, c4_export_list):
    with open(output_dir + os.sep +  'c4.def', 'w') as fout:
        fout.write("EXPORTS\n\n")
        ...
        if(os.path.isfile('c4_def.txt')):
            fout.write("\n; Windows Specific\n")
            for line in open('c4_def.txt', 'r'):
                fout.write(line)

        fout.write("\n")

def write_exp_file(cbl_export_list, c4_export_list):
    with open(output_dir + os.sep + 'c4.exp', 'w') as fout:
        fout.write("# Couchbase Lite .NET\n")
        for cbl_export in cbl_export_list:
            if len(cbl_export) > 0:
                fout.write("_{0}\n".format(cbl_export))
        ...
        if os.path.isfile('c4_exp.txt'):
            fout.write("\n# Apple specific\n")
            for line in open('c4_exp.txt', 'r'):
                fout.write(line)

        fout.write("\n")

def write_gnu_file(cbl_export_list, c4_export_list):
    with open(output_dir + os.sep + 'c4.gnu', 'w') as fout:
        fout.write("{\n\tglobal:\n")
        for cbl_export in cbl_export_list:
            if len(cbl_export) > 0:
                fout.write("\t\t{0};\n".format(cbl_export))
        ...
        if os.path.isfile('c4_gnu.txt'):
            for line in open('c4_gnu.txt', 'r'):
                fout.write(line)
...
if __name__ == "__main__":
    output_dir = os.path.dirname(os.path.realpath(__file__)) + os.sep + ".." + os.sep
