...
namespace litecore {
    ...
    void DatabaseImpl::upgradeDocumentVersioning(C4DocumentVersioning curVersioning, C4DocumentVersioning newVersioning,
        ...
        LogTo(DBLog, "*** Upgrading stored documents from %s to %s ***", kNameOfVersioning[curVersioning],
              kNameOfVersioning[newVersioning]);
        uint64_t docCount = 0;
            ...
            if ( SQLiteDataFile::keyStoreNameIsCollection(ksName) ) {
                ...
                while ( e.next() ) {
                    ...
                    if ( newVersioning == kC4VectorVersioning ) {
                        ...
                        LogVerbose(DBLog, "  - Upgraded doc '%.*s' #%s", SPLAT(rec.key()),
    ...
    static pair<alloc_slice, alloc_slice> upgradeRemoteRevs(DatabaseImpl* db, Record rec, RevTreeRecord& revTree,
        ...
        for ( auto i : revTree.remoteRevisions() ) {
            ...
            if ( rev == currentRev ) {
                ...
                if ( rev->body() ) nuRev.properties = fleece::ValueFromData(rev->body(), kFLTrusted).asDict();
                nuRev.flags = {};
