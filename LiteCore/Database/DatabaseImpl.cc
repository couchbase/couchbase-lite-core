...
#pragma mark - OPENING / CLOSING:
    ...
    /*static*/ FilePath DatabaseImpl::findOrCreateBundle(const string& path, bool canCreate,
        ...
        if ( createdDir || factory->fileExists(dbPath) ) {
            ...
            if ( storageEngine == nullptr ) storageEngine = factory->cname();
            return dbPath;
        }
    ...
    void DatabaseImpl::open(const FilePath& bundlePath) {
        ...
        if ( options.useDocumentKeys ) _encoder->setSharedKeys(_dataFile->documentKeys());

        // Validate or upgrade the database's document schema/versioning:
        _configV1.versioning = checkDocumentVersioning();

        if ( _configV1.versioning == kC4VectorVersioning ) _config.flags |= kC4DB_VersionVectors;
        else
    ...
    DatabaseImpl::~DatabaseImpl() {
        Assert(_transactionLevel == 0, "Database being destructed while in a transaction");
...
#pragma mark - ACCESSORS:
    ...
    uint32_t DatabaseImpl::maxRevTreeDepth() {
        if ( _maxRevTreeDepth == 0 ) {
            ...
            if ( _maxRevTreeDepth == 0 ) _maxRevTreeDepth = kDefaultMaxRevTreeDepth;
        }
        return _maxRevTreeDepth;
    }

    void DatabaseImpl::setMaxRevTreeDepth(uint32_t depth) {
        if ( depth == 0 ) depth = kDefaultMaxRevTreeDepth;
...
#pragma mark - HOUSEKEEPING:
    ...
    void DatabaseImpl::maintenance(C4MaintenanceType what) {
        ...
        if ( what == kC4Compact ) garbageCollectBlobs();
    }
    ...
    void DatabaseImpl::garbageCollectBlobs() {
        ...
        auto                     blobCallback = [&](FLDict blob) {
            if ( auto key = C4Blob::keyFromDigestProperty(blob); key ) usedDigests.insert(*key);
            return true;
        };
        ...
        if ( numDeleted > 0 || !usedDigests.empty() ) {
            LogTo(DBLog, "    ...deleted %u blobs (%zu remaining)", numDeleted, usedDigests.size());
        }
    }
...
#pragma mark - UUIDS:
    ...
    void DatabaseImpl::resetUUIDs() {
        ...
        setInfo(constants::kPreviousPrivateUUIDKey, {&previousPrivate, sizeof(C4UUID)});
...
#pragma mark - COLLECTIONS:
    ...
    static string collectionNameToKeyStoreName(C4Database::CollectionSpec spec) {
        if ( isDefaultCollection(spec) ) return DataFile::kDefaultKeyStoreName;

        if ( !isValidScopeNameOrDefault(spec.scope) )
            C4Error::raise(LiteCoreDomain, kC4ErrorInvalidParameter, "Invalid scope name '%.*s'", SPLAT(spec.scope));
        if ( !KeyStore::isValidCollectionName(spec.name) )
            C4Error::raise(LiteCoreDomain, kC4ErrorInvalidParameter, "Invalid collection name '%.*s' in scope '%.*s'",
    ...
    static C4Database::CollectionSpec keyStoreNameToCollectionSpec(slice name) {
        if ( name == DataFile::kDefaultKeyStoreName ) return {kC4DefaultCollectionName, kC4DefaultScopeID};
    ...
    bool DatabaseImpl::hasCollection(CollectionSpec spec) const {
        ...
        if ( found != _collections.end() && found->second->isValid() ) { return true; }
    ...
    C4Collection* DatabaseImpl::getOrCreateCollection(CollectionSpec spec, bool canCreate) {
        ...
        if ( (!canCreate || isDefaultCollection(spec)) && !_dataFile->keyStoreExists(keyStoreName) ) {
            if ( canCreate && isDefaultCollection(spec) )
                C4Error::raise(LiteCoreDomain, kC4ErrorInvalidParameter, "You cannot recreate the default collection");
            return nullptr;  //-> NULL
        }
    ...
    void DatabaseImpl::forEachScope(const ScopeCallback& callback) const {
        ...
        forEachCollection([&](CollectionSpec spec) {
            if ( seenScopes.insert(spec.scope).second ) callback(spec.scope);
        });
    }
...
#pragma mark - TRANSACTIONS:
    ...
    void DatabaseImpl::endTransaction(bool commit) {
        ...
        if ( _transactionLevel == 0 ) error::_throw(error::NotInTransaction);
...
#if DEBUG
            case kArray:
                for ( Array::iterator j(val->asArray()); j; ++j ) validateKeys(j.value(), false);
                break;
#endif
            case kDict:
                {
                    ...
                    for ( Dict::iterator i(d); i; ++i ) {
                        ...
                        if ( !key.buf || d->get(key) != i.value() )
                            error::_throw(error::CorruptRevisionData, "Document key is not properly encoded");
                        if ( atRoot && (key == "_id"_sl || key == "_rev"_sl || key == "_deleted"_sl) )
                            error::_throw(error::CorruptRevisionData, "Illegal top-level key `%.*s` in document",
                                          SPLAT(key));
#if DEBUG
                        if ( i.key()->asString() && val->sharedKeys()->couldAdd(key) )
                            error::_throw(error::CorruptRevisionData, "Key `%.*s` should have been shared-key encoded",
                                          SPLAT(key));
    ...
    C4RemoteID DatabaseImpl::getRemoteDBID(slice remoteAddress, bool canCreate) {
        ...
        for ( int creating = 0; creating <= 1; ++creating ) {
            ...
            if ( doc.exists() ) {
                ...
                if ( body ) remotes = body->asDict();
                if ( remotes ) {
                    auto idObj = remotes->get(remoteAddress);
                    if ( idObj ) remoteID = C4RemoteID(idObj->asUnsigned());
                }
            }
        ...
        if ( Record doc = getInfo(kRemoteDBURLsDoc); doc.exists() ) {
            ...
            if ( body ) {
                for ( impl::Dict::iterator i(body->asDict()); i; ++i ) {
                    if ( i.value()->asInt() == remoteID ) return alloc_slice(i.keyString());
                }
            }
        }
        return nullslice;
