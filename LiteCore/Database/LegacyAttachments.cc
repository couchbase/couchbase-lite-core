...
namespace litecore { namespace legacy_attachments {
    ...
    bool hasOldMetaProperties(const Dict* root, SharedKeys *sk) {
    ...
    alloc_slice encodeStrippingOldMetaProperties(const Dict *root, SharedKeys *sk) {
        ...
        auto attachments = Value::asDict(root->get(C4STR(kC4LegacyAttachmentsProperty), sk));
        if (attachments) {
            for (Dict::iterator i(attachments, sk); i; ++i) {
                ...
                if (attDigest && blob && Document::dictIsBlob(blob, sk)) {
                    ...
                    removeThese.insert(attachment);
                    auto blobDigest = blob->get("digest"_sl, sk);
        ...
        Encoder enc;
        enc.writeValue(root, sk, [&](const Value *key, const Value *value) {
            ...
            if (b != fixBlobs.end()) {
                ...
                for (Dict::iterator i(blob, sk); i; ++i) {
                    ...
                    auto attValue = attachment->get(blobKey, sk);
                    if (attValue || blobKey == "length"_sl || blobKey == "content_type"_sl)
                        blobValue = attValue;
                    if (blobValue) {
                        enc.writeKey(i.key(), sk);
