...
namespace fleece {
    ...
    slice DateFormat::format(char buf[], const int64_t timestamp, const minutes tzoffset,
        ...
        if ( f.ymd.has_value() ) { stream << fleece::format("%F", tm); }
        ...
        if ( f.hms.has_value() ) {
            ...
            if ( f.tz.has_value() ) {
                if ( tzoffset.count() == 0 ) {
                    ...
                    // Not routed through fleece::format: this is plain integer formatting (no
                    // chrono formatter involved), so it has no platform variance to guard against.
                    char     sign = tzoffset.count() < 0 ? '-' : '+';
                    hh_mm_ss hms{sign == '-' ? -tzoffset : tzoffset};
                    stream << sign << std::setfill('0') << std::setw(2) << hms.hours().count();
                    if ( f.tz.value() == Timezone::Colon ) stream << ":";
                    stream << std::setfill('0') << std::setw(2) << hms.minutes().count();
                }
            }
        }
