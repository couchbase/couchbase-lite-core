    ...
    bool SQLiteKeyStore::isIndexTrained(fleece::slice name) const {
        ...
        for (const auto& spec : specs) {
            if( name == spec.name) {
                ...
                auto vectorTableName = db().auxiliaryTableName(tableName(), KeyStore::kVectorSeparator, (string)name) + vectorTableNameSuffix;
                auto rawResult = db().rawQuery(format("SELECT tableName FROM %s WHERE tableName = '%s'", vectorDataTableName, vectorTableName.c_str()));
