...
#pragma mark - SOURCE:
    ...
    IndexType SourceNode::indexType() const {
        return isIndex() ? _indexedNodes[0]->indexType() : IndexType::none;
    }
...
#pragma mark - SELECT:
        ...
        if (select) {
            ...
            if (Value what = getCaseInsensitive(select, "WHAT")) {
                for (Value w : requiredArray(what, "WHAT")) {
                    ...
                    if (whatNode->hasExplicitAlias())
                        registerAlias(whatNode.get(), ctx);
            ...
            if (Value limit = getCaseInsensitive(select, "LIMIT"))
                _limit = ExprNode::parse(limit, ctx);
            if (Value offset = getCaseInsensitive(select, "OFFSET"))
                _offset = ExprNode::parse(offset, ctx);
    ...
    void SelectNode::addAllSourcesTo(std::vector<SourceNode *> &sources) const {
        ...
        for (auto sel : _nestedSelects)
            sel->addAllSourcesTo(sources);
    }
    ...
    Node* SelectNode::postprocess(ParseContext& ctx) {
        ...
        visit([&](Node& node, unsigned depth) {
            if (auto meta = dynamic_cast<MetaNode*>(&node)) {
                ...
                if (prop == MetaProperty::none || prop == MetaProperty::deleted)
                    meta->source()->setUsesDeleted();
            } else if (auto fn = dynamic_cast<FunctionNode*>(&node)) {
                // Look for aggregate functions:
                if (fn->opFlags() & kOpAggregate)
                    _isAggregate = true;
            }
        });
        ...
        for (auto& source : _sources)
            source->disambiguateColumnName(ctx);

        {
            ...
            for (int x = 1; x >= 0; --x) {
                for (auto& what : _what) {
                    if (what->hasExplicitAlias() == x)
                        what->ensureUniqueColumnName(columnNames);
                }
            }
        }
    ...
    QueryNode::QueryNode(string_view json)
    :QueryNode(Doc::fromJSON(json).root())
    { }
