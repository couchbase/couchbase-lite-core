...
namespace litecore {
    ...
    QueryTranslator::QueryTranslator(const Delegate& delegate, 
    ...
    void QueryTranslator::parse(FLValue v) {
        ...
        for (auto& what : query->what())
            _columnTitles.push_back(what->columnName());
        ...
        query->visit([this](Node& node, unsigned depth) {
            if (auto p = dynamic_cast<ParameterNode*>(&node)) {
                ...
                if (m->property() == MetaProperty::expiration)
    ...
    string QueryTranslator::expressionSQL(FLValue exprSource) {
        ...
        return writeSQL([&](SQLWriter& writer) {
            writer << *expr;
        });
    }
...
#pragma mark - INDEX CREATION:
    ...
    void QueryTranslator::writeCreateIndex(const string& indexName,
                          const string& onTableName,
                          FLArrayIterator& whatExpressions,
                          FLArray whereClause,
    ...
    string QueryTranslator::functionCallSQL(slice fnName, FLValue args) {
        ...
        return writeSQL([&](SQLWriter& writer) {
            writeFnGetter(fnName, *expr, nullptr, writer);
        });
    }
