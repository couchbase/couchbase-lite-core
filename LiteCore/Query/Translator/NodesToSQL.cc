...
namespace litecore::qt {
    ...
    void RawSQLNode::writeSQL(SQLWriter &ctx) const {
        ctx << _sql;
    }
    ...
    void MetaNode::writeSQL(SQLWriter &ctx) const {
        string aliasDot;
        if (_source)
            aliasDot = CONCAT( sqlIdentifier(_source->alias()) << "." );
    ...
    void ParameterNode::writeSQL(SQLWriter& ctx) const {
        ...
        switch (_op.type) {
            ...
            case OpType::is_not: {
                ...
                if (auto lit = dynamic_cast<LiteralNode*>(_operands[1].get())) {
                    ...
                    if (lit->literal().type() == kFLNull)
                        opName = (_op.type == OpType::is) ? "=" : "!=";
                }
