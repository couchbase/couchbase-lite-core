...
N_WAY_TEST_CASE_METHOD(QueryTest, "Query Date Functions", "[Query][CBL-59]") {
    ...
    mil_to_str << fleece::format("%FT%T", mil_to_str_time + offset_seconds);
    if ( offset_seconds.count() == 0 ) {
        mil_to_str << "Z";
    } else {
        // Plain integer formatting (no chrono formatter involved), so no platform variance.
        char     sign = offset_seconds.count() < 0 ? '-' : '+';
        hh_mm_ss hms{sign == '-' ? -offset_seconds : offset_seconds};
        mil_to_str << sign << setfill('0') << setw(2) << hms.hours().count() << setw(2) << hms.minutes().count();
    }
    ...
    s1 << fleece::format("%F", utc_time);
    s1iso << fleece::format("%FT%TZ", utc_time);
    utc_time += 18h + 33min;
    s2 << fleece::format("%FT%TZ", utc_time);
    s2iso << fleece::format("%FT%TZ", utc_time);
    utc_time += 1s;
    s3 << fleece::format("%FT%T", utc_time);
    s3iso << fleece::format("%FT%TZ", utc_time);
    s5 << fleece::format("%FT%TZ", utc_time);
    ...
    stringstream s4;
    s4 << fleece::format("%FT%TZ", utc_time);
