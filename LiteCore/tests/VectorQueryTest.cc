...
TEST_CASE_METHOD(SIFTVectorQueryTest, "Index isTrained API", "[Query][.VectorSearch]") {
...

    {
        ...
        try {
            ...
            CHECK(e == error::InvalidParameter);
        }
    }

    // Need to run an arbitrary query to actually train the index
    string queryStr = R"(SELECT META().id, publisher FROM )"s + collectionName + R"( WHERE VECTOR_MATCH(vecIndex, $target, 5) )";
