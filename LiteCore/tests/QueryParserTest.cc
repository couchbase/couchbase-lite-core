...
TEST_CASE_METHOD(QueryParserTest, "QueryParser SELECT UNNEST", "[Query][QueryParser][FTS]") {
    CHECK(parseWhere("['SELECT', {\
                      FROM: [{as: 'book'}, \
...
TEST_CASE_METHOD(QueryParserTest, "QueryParser SELECT UNNEST optimized", "[Query][QueryParser][FTS]") {
