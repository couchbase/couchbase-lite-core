...
N_WAY_TEST_CASE_METHOD(QueryTest, "Predictive Query", "[Query][Predict]") {
    ...
    Retained<Query> query{
            store->compileQuery(json5("{'WHAT': [['._id'], ['PREDICTION()', '8ball', {number: ['.num']}]]}"))};
    testResults(query);
...
N_WAY_TEST_CASE_METHOD(QueryTest, "Predictive Query invalid input", "[Query][Predict]") {
    ...
    Retained<Query> query{store->compileQuery(json5("{'WHAT': [['.value'], ['PREDICTION()', '8ball', ['.value']]]}"))};
    ExpectException(error::SQLite, 1, [&]() { Retained<QueryEnumerator> e(query->createEnumerator()); });
...
N_WAY_TEST_CASE_METHOD(QueryTest, "Predictive Query indexed", "[Query][Predict]") {
    ...
    for ( int pass = 1; pass <= 3; ++pass ) {
        ...
        if ( pass > 1 ) {
            store->createIndex("nums"_sl, json5("[" + prediction + "]"), IndexSpec::kPredictive);
        ...
        Retained<QueryEnumerator> e(query->createEnumerator());
        while ( e->next() ) {
            if ( e->columns()[0]->type() == kNumber ) results.push_back(e->columns()[0]->asInt());
        }
        CHECK(results
              == (vector<int64_t>({100, 81, 64, 49, 36, 25, 16, 9,  4,  1,  99, 82, 80, 65, 63, 50, 48, 37, 35, 26,
                                   98,  24, 83, 79, 17, 66, 62, 15, 51, 47, 97, 10, 38, 84, 78, 34, 8,  67, 61, 27,
                                   96,  23, 52, 46, 85, 77, 5,  18, 39, 68, 95, 60, 33, 14, 3,  86, 53, 76, 28, 45,
...
N_WAY_TEST_CASE_METHOD(QueryTest, "Predictive Query compound indexed", "[Query][Predict]") {
    ...
    for ( int pass = 1; pass <= 3; ++pass ) {
        ...
        if ( pass > 1 ) {
            ...
            store->createIndex("nums"_sl, json5("[" + index + "]"), IndexSpec::kPredictive);
