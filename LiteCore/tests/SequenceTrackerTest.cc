...
TEST_CASE_METHOD(litecore::SequenceTrackerTest, "SequenceTracker", "[notification]") {
    ...
    tracker.documentChanged("A"_id, "1-aa"_rev, ++seq, 1111);
    tracker.documentChanged("B"_id, "1-bb"_rev, ++seq, 2222);
    tracker.documentChanged("C"_id, "1-cc"_rev, ++seq, 3333);
    REQUIRE_IF_DEBUG(dump(true) == "[(A@1#1111, B@2#2222, C@3#3333)]");
    CHECK(tracker.lastSequence() == seq);
    tracker.documentChanged("B"_id, "2-bb"_rev, ++seq, 4444);
    REQUIRE_IF_DEBUG(dump(true) == "[(A@1#1111, C@3#3333, B@4#4444)]");
    tracker.documentChanged("B"_id, "3-bb"_rev, ++seq, 5555);
    CHECK(tracker.lastSequence() == seq);
    REQUIRE_IF_DEBUG(dump(true) == "[(A@1#1111, C@3#3333, B@5#5555)]");
    tracker.documentChanged("A"_id, "2-aa"_rev, ++seq, 6666);
    CHECK(tracker.lastSequence() == seq);
    REQUIRE_IF_DEBUG(dump(true) == "[(C@3#3333, B@5#5555, A@6#6666)]");
    tracker.documentChanged("D"_id, "1-dd"_rev, ++seq, 7777);
    CHECK(tracker.lastSequence() == seq);
...
TEST_CASE_METHOD(litecore::SequenceTrackerTest, "SequenceTracker DatabaseChangeNotifier", "[notification]") {
    ...
    DatabaseChangeNotifier cn2(tracker, [&](DatabaseChangeNotifier&) {++count2;});
    {
        ...
        tracker.documentChanged("C"_id, "2-cc"_rev, ++seq, 5555);
...
TEST_CASE_METHOD(litecore::SequenceTrackerTest, "SequenceTracker DocChangeNotifier", "[notification]") {
    ...
    DocChangeNotifier cnA(tracker, "A"_id, [&](DocChangeNotifier&, DocID docID, sequence_t s) {
        CHECK(docID == "A"_sl);
        CHECK(s == seq);
        ++countA;
    });
    DocChangeNotifier cnB(tracker, "B"_id, [&](DocChangeNotifier&, DocID docID, sequence_t s) {
        CHECK(docID == "B"_sl);
        CHECK(s == seq);
        ++countB;
    });
    // Create one for a doc that doesn't exist yet:
    DocChangeNotifier cnD(tracker, "D"_id, [&](DocChangeNotifier&, DocID docID, sequence_t s) {
        CHECK(docID == "D"_sl);
        CHECK(s == seq);
        ++countD;
    });

    tracker.documentChanged("A"_id, "2-aa"_rev, ++seq, 4444);
    CHECK(countA==1);
    CHECK(countB==0);

    tracker.documentChanged("B"_id, "2-bb"_rev, ++seq, 5555);
    CHECK(countA==1);
    CHECK(countB==1);

    {
        DocChangeNotifier cnB2(tracker,"B"_id, [&](DocChangeNotifier&,DocID,sequence_t) {++countB2;});
        tracker.documentChanged("B"_id, "3-bb"_rev, ++seq, 6666);
        CHECK(countA==1);
        CHECK(countB==2);
        CHECK(countB2==1);
    }

    tracker.documentChanged("B"_id, "4-bb"_rev, ++seq, 7777);
    CHECK(countA==1);
    CHECK(countB==3);
    CHECK(countB2==1);
    CHECK(countD==0);

    tracker.documentChanged("D"_id, "1-dd"_rev, ++seq, 8888);
    CHECK(countA==1);
    CHECK(countB==3);
    CHECK(countB2==1);
    CHECK(countD==1);

    tracker.documentChanged("Z"_id, "9-zz"_rev, ++seq, 999);
...
TEST_CASE("SequenceTracker Transaction", "[notification]") {
    ...
    tracker.beginTransaction();
    tracker.documentChanged("B"_id, "2-bb"_rev, ++seq, 4444);
    tracker.documentChanged("D"_id, "1-dd"_rev, ++seq, 5555);
    ...
    DocChangeNotifier cnA(tracker, "A"_id, [&](DocChangeNotifier&,DocID,sequence_t) {
        ++countA;
    });
    DocChangeNotifier cnB(tracker, "B"_id, [&](DocChangeNotifier&,DocID,sequence_t) {
        ++countB;
    });
    DocChangeNotifier cnD(tracker, "D"_id, [&](DocChangeNotifier&,DocID,sequence_t) {
        ++countD;
    });
...
TEST_CASE_METHOD(litecore::SequenceTrackerTest, "SequenceTracker Ignores ExternalChanges", "[notification]") {
    ...
    track2.documentChanged("B"_id, "2-bb"_rev, ++seq, 4444);
    track2.documentChanged("Z"_id, "1-ff"_rev, ++seq, 5555);
