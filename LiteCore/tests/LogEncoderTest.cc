...
TEST_CASE("Logging rollover", "[Log]") {
    ...
    int totalCount = 0;
    vector<string> whatFiles;
    tmpLogDir.forEachFile([&infoFiles, &totalCount, &whatFiles](const FilePath f)
    {
       totalCount++;
       if(f.path().find("info") != string::npos) {
           infoFiles.push_back(f.path());
       } 
        whatFiles.push_back(f.path());
    });

    if (totalCount != 8) {
        std::stringstream ss;
        ss << "[Logging rollever] expects 8 files, actual files are:";
        for (auto& p: whatFiles) {
            ss << "\n\t" << p;
        }
        printf("%s\n", ss.str().c_str());
    } else {
        printf("[Logging rollever] passes file count test.\n");
    }
