...
TEST_CASE_METHOD(N1QLParserTest, "N1QL JOIN", "[Query][N1QL][C]") {
    ...
    CHECK(translate("SELECT rec, dss, dem FROM db rec JOIN db dss ON rec.sessionId = dss.meta.id "
                    "JOIN db dem ON rec.demId = dem.meta.id WHERE rec.meta.id LIKE 'rec:%'")
          == "{'FROM':[{'AS':'rec'},{'AS':'dss','JOIN':'INNER','ON':['=',['.rec.sessionId'],['.dss._id']]},{'AS':'dem','JOIN':'INNER',"
             "'ON':['=',['.rec.demId'],['.dem._id']]}],'WHAT':[['.rec'],['.dss'],['.dem']],'WHERE':['LIKE',['.rec._id'],'rec:%']}");
