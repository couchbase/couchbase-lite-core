...
namespace litecore {
    ...
    public:
        ...
        explicit VersionVecWithLegacy(revid revID) {
            if (revID) {
                if (revID.isVersion())
                    vector = VersionVector::fromBinary(revID);
                else
        ...
        void sortLegacy() {
            std::sort(legacy.begin(), legacy.end(), [](auto& r1, auto& r2) {
                return revid(r1).generation() > revid(r2).generation();
            });
        }
        ...
        static versionOrder compare(VersionVecWithLegacy const& a, VersionVecWithLegacy const& b) {
            ...
            if (vectorOrder == kXConflicting || legacyOrder == kXConflicting)
                return kConflicting;
            else
    ...
    private:
        ...
        void parse(const slice history[], size_t historyCount, SourceID mySourceID) {
            ...
            while (historyCount > 0) {
                ...
                if (lastHistory.getRevID().isVersion() )
                    break;
        ...
        static extendedVersionOrder extendedCompare(V const& a, V const& b) {
            ...
            if (emptyOrder != kConflicting)
                return extendedVersionOrder(emptyOrder);    // -> one or both is empty
            else
                return extendedVersionOrder(3 + _compare(a, b)); // -> neither empty; do a proper compare
        }


        static versionOrder _compare(VersionVector const& a, VersionVector const& b) {
            return a.compareTo(b);
        }

        /// Compares two legacy revision histories. They must be non-empty.
        static versionOrder _compare(std::vector<alloc_slice> const& a,
                                     std::vector<alloc_slice> const& b)
        {
