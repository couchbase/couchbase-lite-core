...
#pragma mark - CONVERSION:
        ...
        if ( count() > 1 ) {
            for ( auto i = next(_vers.begin()); i != _vers.end(); ++i ) {
                ...
                for ( auto j = _vers.begin(); j != i; ++j )
                    if ( j->author() == author ) error::_throw(error::BadRevisionID, "Duplicate ID in version vector");
            }
        }
    }
#endif
    ...
    void VersionVector::readBinary(slice data) {
        ...
        while ( in.size > 0 ) _vers.emplace_back(in);
        validate();
    }
    ...
    void VersionVector::readHistory(const slice history[], size_t historyCount, peerID myPeerID) {
        ...
        if ( historyCount == 2 ) {
            ...
            for ( size_t i = 1; i < historyCount; ++i ) {
                ...
                if ( auto gen = genOfAuthor(parentVers.author()); gen == 0 ) _vers.push_back(parentVers);
                else if ( gen <= parentVers.gen() )
                    error::_throw(error::BadRevisionID, "Invalid version history (increasing generation)");
            }
        }  // -> List of versions
    }
...
#pragma mark - OPERATIONS:
    ...
    versionOrder VersionVector::compareTo(const VersionVector& other) const {
        ...
        if ( countDiff < 0 ) o = kOlder;  // other must have versions from authors I don't have
        else if ( countDiff > 0 )
        ...
        for ( auto& v : _vers ) {
            ...
            if ( v.gen() < othergen ) {
                ...
                if ( othergen == 0 ) {
                    ...
                    if ( --countDiff < 0 ) o = versionOrder(o | kOlder);
                }
            }
            if ( o == kConflicting ) break;
        }
        return o;
    }
...
#pragma mark - MODIFICATION:
    ...
    void VersionVector::limitCount(size_t maxCount) {
        if ( _vers.size() > maxCount ) _vers.erase(_vers.begin() + maxCount, _vers.end());
    }
    ...
    bool VersionVector::add(Version v) {
        if ( auto versP = findPeerIter(v.author()); versP != _vers.end() ) {
            if ( versP->gen() >= v.gen() ) return false;
            _vers.erase(versP);
        }
    ...
    void VersionVector::compactMyPeerID(peerID myID) {
        if ( genOfAuthor(kMePeerID) > 0 ) error::_throw(error::BadRevisionID, "Vector already contains '*'");
        auto versP = findPeerIter(myID);
        if ( versP != _vers.end() ) *versP = Version(versP->gen(), kMePeerID);
    }

    void VersionVector::expandMyPeerID(peerID myID) {
        if ( genOfAuthor(myID) > 0 ) error::_throw(error::BadRevisionID, "Vector already contains myID");
        auto versP = findPeerIter(kMePeerID);
        if ( versP != _vers.end() ) *versP = Version(versP->gen(), myID);
    }

    bool VersionVector::isExpanded() const { return genOfAuthor(kMePeerID) == 0; }
