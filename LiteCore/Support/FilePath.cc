...
namespace litecore {
    ...
    pair<string, string> FilePath::splitPath(string_view path) {
        ...
        if ( slash == string::npos && backupSlash == string::npos ) { return {kCurrentDir, string(path)}; }
    ...
    string FilePath::unextendedName() const { return splitExtension(fileOrDirName()).first; }
    ...
    string FilePath::fileOrDirName() const {
        ...
        auto path = _dir;
        if ( path.size() <= 1 || path == kCurrentDir ) return "";
    ...
    FilePath FilePath::parentDir() const {
        ...
        if ( p == kCurrentDir ) error::_throw(error::POSIX, EINVAL);
#ifdef _MSC_VER
...
#pragma mark - OPERATIONS:
    ...
    static inline void check(int result) {
        if ( _usuallyFalse(result != 0) ) error::_throwErrno();
    }
    ...
    void FilePath::mustExistAsDir() const {
        ...
        if ( !S_ISDIR(s.st_mode) ) error::_throw(error::POSIX, ENOTDIR);
    }

    bool FilePath::mkdir(int mode) const {
        if ( mkdir_u8(path().c_str(), mode) != 0 ) {
            if ( errno != EEXIST ) error::_throwErrno();
            return false;
        }
        return true;
    }
    ...
    FilePath FilePath::mkTempDir() const {
        char pathBuf[kPathBufSize];
        makePathTemplate(this, pathBuf);
        if ( mkdtemp(pathBuf) == nullptr ) error::_throwErrno();
    ...
    void FilePath::setReadOnly(bool readOnly) const { chmod_u8(path().c_str(), (readOnly ? 0400 : 0600)); }
