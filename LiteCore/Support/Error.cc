...
namespace litecore {
    ...
    static string cbl_strerror(int err) {
        ...
        static long wsaEquivalent[] = {WSAEADDRINUSE,
                                       WSAEADDRNOTAVAIL,
                                       WSAEAFNOSUPPORT,
                                       WSAEALREADY,
                                       0,
...
#ifdef WIN32
#    define MAPWSA(code)                                                                                               \
        { WSA##code, error::POSIX, code }
#    define MAPWSATO(from, to)                                                                                         \
        { from, error::POSIX, to }

    static const codeMapping kPrimaryCodeMapping[] = {MAPWSA(EADDRINUSE),
    ...
    __cold static int getPrimaryCode(const error::Domain& domain, const int& code) {
        ...
        if ( domain != error::Domain::SQLite ) { return code; }
    ...
    __cold static const char* websocket_errstr(int code) {
        ...
        for ( unsigned i = 0; kWebSocketMessages[i].message; ++i ) {
            if ( kWebSocketMessages[i].code == code ) return kWebSocketMessages[i].message;
        }
            ...
            case SQLite:
                {
                    ...
                    if ( code == primary ) { return sqlite3_errstr(code); }
...
#    define sCaptureBacktraces c4error_getCaptureBacktraces()
    ...
    __cold void error::captureBacktrace(unsigned skipFrames) {
        if ( !backtrace ) backtrace = fleece::Backtrace::capture(skipFrames + 1);
    }

    __cold error error::standardized() const {
        Domain d = domain;
        int    c = code;
        switch ( domain ) {
            case POSIX:
    ...
    __cold void error::_throw(unsigned skipFrames) {
        if ( sWarnOnError && !isUnremarkable() ) {
            if ( sNotableExceptionHook ) sNotableExceptionHook();
            captureBacktrace(2 + skipFrames);
            WarnError("LiteCore throwing %s error %d: %s\n%s", nameOfDomain(domain), code, what(),
                      backtrace->toString().c_str());
        }
        throw *this;
    }

    __cold void error::_throw(Domain domain, int code) { error{domain, code}._throw(1); }

    __cold void error::_throw(error::LiteCoreError err) { error{LiteCore, err}._throw(1); }
